...
import com.liutf.mvc.entity.AccessToken;
    ...
    public static AccessToken getAccessToken(String appid, String appsecret)
        ...
        if (null != json) {
            try {
                ...
                accessToken.setAccessToken(json.get("access_token")
                        .getAsString());
    ...
    public static void main(String[] args) {
		...
		try {
			...
			PropertiesUtil putil = new PropertiesUtil(
					Constants.CONF_PROPERTIES_PATH);
			Long overduetime = Long.parseLong(putil
					.getProperty("OVERDUE_ACCESS_TOKEN_TIME"));
			Timestamp s = new Timestamp();
			AccessToken token = new AccessToken();
			if (s.getDateTime() > overduetime) {
				// 超时从新获取accessToken
				token = util.getAccessToken2(Constants.APPID, Constants.SECRET);
				putil.writeProperties(
						"OVERDUE_ACCESS_TOKEN_TIME",
						String.valueOf(s.getDateTime()
